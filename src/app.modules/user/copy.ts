...
//     const status = this.buildUserStatus(userId, currentCompanyId, dto.userId);
...
//     targetUserId: number,
//   ): string {
//     // For simplicity, a placeholder is returned.
//     return 'Status info based on relationship';
//   }
...
//       await this.prisma.friendship.update({ where: { id: request.id }, data: { wasLoadedByReceiver: true } });
//     });
//     teamInvitations.forEach(async request => {
//       await this.prisma.teamInvitation.update({ where: { id: request.id }, data: { wasLoadedByReceiver: true } });
//     });
//     return notifications;
//   }
